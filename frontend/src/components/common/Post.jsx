					...
					<div className='flex justify-between mt-3 text-white'>
						<div className='flex gap-4 items-center w-2/3 justify-between text-white'>
							...
							>
								...
								<span className='text-sm text-white group-hover:text-sky-400'>
									{post.comments.length}
								</span>
							</div>
							{/* We're using Modal Component from DaisyUI */}
							<dialog id={`comments_modal${post._id}`} className='modal border-none outline-none'>
								<div className='modal-box rounded border border-white'>
									...
									<form
										className='flex gap-2 items-center mt-4 border-t border-white pt-2'
										onSubmit={handlePostComment}
									>
										<textarea
											className='textarea w-full p-1 rounded text-md resize-none border focus:outline-none  border-white'
											placeholder='Add a comment...'
											value={comment}
											onChange={(e) => setComment(e.target.value)}
										/>
										<button className='bg-blue-500 hover:bg-blue-700 rounded-full btn-sm text-white px-4'>
							...
							<div className='flex gap-1 items-center group cursor-pointer' onClick={handleLikePost}>
								...
								{!isLiked && !isLiking && (
									<FaRegHeart className='w-4 h-4 cursor-pointer text-white group-hover:text-pink-500' />
								)}
								{isLiked && !isLiking && (
